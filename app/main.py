...
def _catalog_discovery_runtime(addon_urls: list[str] | None = None):
    """Build a metadata runtime for catalog discovery.

    When the dashboard supplies addon URLs, use those live values instead of
    requiring them to be saved to SQLite first.
    """
    runtime = _runtime()
    ...
    cleaned: list[str] = []
    for raw_url in addon_urls:
        manifest_url = _normalize_manifest_url(str(raw_url or ""))
        if not manifest_url:
            continue
        base_url = _addon_base_url(manifest_url)
        if base_url and base_url not in cleaned:
            cleaned.append(base_url)

    return runtime.model_copy(
        update={"stremio_addon_urls": ",".join(cleaned)}
    )


async def _discover_catalogs_for_runtime(runtime, *, force: bool = False) -> list[dict]:
    """Discover catalogs, optionally bypassing the shared manifest cache."""
    service = MetadataService(runtime)

    if force:
        addon_urls = [
            service._base_url(url)
            for url in runtime.addon_urls
            if service._base_url(url)
        ]
        cache_key = "manifests:" + "|".join(addon_urls)
        await metadata_cache.invalidate(cache_key)

    return await service.manifests()


@app.get("/api/addons/catalogs")
async def discover_catalogs(request: Request, force: bool = False):
    if (denied := await _require(request)):
        return denied

    runtime = _catalog_discovery_runtime()
    return {
        "catalogs": await _discover_catalogs_for_runtime(
            runtime,
            force=force,
        )
    }
...
@app.post("/api/addons/catalogs")
async def discover_catalogs_post(request: Request):
    """Discover catalogs from the dashboard's current unsaved addon list."""
    if (denied := await _require(request)):
        return denied
    ...
    raw_urls = body.get("addon_urls", [])
    if not isinstance(raw_urls, list):
        return JSONResponse(
            {"detail": "addon_urls must be a list"},
            status_code=400,
        )

    force = bool(body.get("force", True))
    runtime = _catalog_discovery_runtime(
        [str(url or "") for url in raw_urls]
    )
